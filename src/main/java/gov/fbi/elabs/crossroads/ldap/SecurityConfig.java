...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		//UsernamePasswordAuthenticationToken token = (UsernamePasswordAuthenticationToken) auth.getObject();
	...
	/*	System.out.println("LDAP Auth starting....");
		auth.ldapAuthentication().userSearchFilter("(sAMAccountName={0})").userSearchBase("dc=elab,dc=local")
				.groupSearchBase("OU=Users,OU=FA_Employees,DC=elab,DC=local").contextSource()
				.url("ldap://38.127.196.10").port(389).managerDn("student9").managerPassword("Octo123!@#");
		try {
			System.out.println("value >>>>>>> {0}   {1}");
		System.out.println("LDAP Auth ending...."+SecurityContextHolder.getContext());
		System.out.println("LDAP Auth ending...."+SecurityContextHolder.getContext().getAuthentication());
		System.out.println("LDAP Auth ending...."+SecurityContextHolder.getContext().getAuthentication().getPrincipal());
